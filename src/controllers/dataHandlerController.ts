...
export class DataHandlerController {
    ...
    async handleIncomingData(req: Request, res: Response): Promise<void> {
        try {
            ...
            const destinations = await this.destinationModel.findByAccountId(account.id);
            ...
            try {
                ...
                res.json({
                    ...
                    response: results.map(r => ({ status: r.status, data: r.data }))
                } as ApiResponse);
            } catch (error: any) {
                res.status(500).json({
                    success: false,
    ...
    private async sendToDestination(destination: any, data: IncomingDataRequest): Promise<any> {
        try {
            ...
            if (httpMethod === 'GET') {
                ...
                response = {statue:200,data:{url:`${url}?${queryParams.toString()}`, incomingData:data, destination}}
            } else {
                ...
                response = {statue:200,data:{url, incomingData:data, destination}}
